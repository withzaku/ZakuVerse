...
type MethologyLiveSearchProps = {
  methologyDocs: MethologyDoc[];
};
...
export function MethologyLiveSearch({ methologyDocs }: MethologyLiveSearchProps) {
  const [query, setQuery] = useState("");
  const searchInputRef = useRef<HTMLInputElement>(null);
  const hasTypedQuery = query.trim().length > 0;

  const filteredMethologyDocs = useMemo(
    () => methologyDocs.filter((doc) => matchesFlexibleQuery(query, [doc.title, doc.focus, doc.keywords, doc.href])),
    [methologyDocs, query],
  );
  ...
  const suggestionItems = useMemo(() => {
    ...
    filteredMethologyDocs.forEach((doc) => {
  ...
  }, [filteredMethologyDocs]);

  return (
    <section className="pt-20 sm:pt-24 lg:pt-28 pb-16 sm:pb-20">
      <Container>
        <div className="max-w-5xl space-y-5">
          <p className="type-label text-primary">Methology</p>
          ...
          <p className="type-body max-w-4xl text-white/74">
            This hub contains service-specific methology documents focused on what to prioritize for stronger SEO
          ...
          <div className="mt-6 border border-primary/28 bg-primary/[0.07] p-5 shadow-[0_20px_55px_rgba(0,0,0,0.45)] sm:p-6">
            ...
            <h2 className="mt-2 font-heading text-2xl font-semibold uppercase tracking-[-0.02em] text-white sm:text-3xl">
              Find the right methology while typing
            </h2>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <label htmlFor="methology-live-search" className="sr-only">
                Search methology documents
              </label>
              <input
                id="methology-live-search"
                ref={searchInputRef}
                value={query}
            ...
            {hasTypedQuery ? (
              <p className="mt-4 text-sm leading-relaxed text-white/76">
                Showing {filteredMethologyDocs.length} matching methology documents for &quot;{query}&quot;.
              </p>
            ) : null}
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          {hasTypedQuery && !filteredMethologyDocs.length ? (
        ...
        <aside className="mt-10 border border-primary/24 bg-primary/10 p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-relaxed text-white/76 sm:text-base">
            These methology files define strategic priorities only: what to do, what to focus on, what to measure, and
